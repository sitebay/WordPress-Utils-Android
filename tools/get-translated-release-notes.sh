...
function fetch() {
  ...
  mkdir -p $TMPDIR
  curl -sSfL --globoff -o $TMPDIR/strings-$code.xml "http://translate.sitebay.org/projects/apps/android/release-notes/$code/default/export-translations?filters[status]=current&format=android" || (echo Error downloading $code)
  done
}
